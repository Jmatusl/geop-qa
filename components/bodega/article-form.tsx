...
import { z } from "zod";
import { Loader2, Save, Wand2, X, CheckCircle2, Paperclip } from "lucide-react";
...
import { ArticleFileUploader } from "./article-file-uploader";
...
export function ArticleForm({ initialData, onSubmit: externalSubmit, isLoading = false, onSuccess, onCancel, inDialog = false, compactMode = false }: ArticleFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      articleType: initialData?.articleType || "repuesto",
      ...
      technicalFilePath: initialData?.technicalFilePath || null,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className={cn("w-full space-y-4", !inDialog && "pb-20 lg:pb-0")} autoComplete="off">
        {/* Sección de campos */}
        <Tabs defaultValue="general" className="w-full">
          <TabsList className="mb-6 grid w-full grid-cols-2 bg-slate-100/80 p-1 dark:bg-slate-800/50 h-11">
            <TabsTrigger value="general" className="flex items-center gap-2 font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm dark:data-[state=active]:bg-slate-950">
              <span>Información General</span>
              <Badge variant="secondary" className="ml-1 bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 text-[10px] py-0 px-1.5 h-5">
                {initialData?.unit || "1 und"}
              </Badge>
            </TabsTrigger>
            <TabsTrigger value="adjuntos" className="flex items-center gap-2 font-bold data-[state=active]:bg-white data-[state=active]:shadow-sm dark:data-[state=active]:bg-slate-950">
          ...
          <TabsContent value="general">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[11px] font-bold uppercase">Unidad de Medida</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value || "UNI"}>
                      <FormControl>
                        <SelectTrigger className="w-full">
                      ...
                      <SelectContent>
                        <SelectItem value="UNI">Unidad (UNI)</SelectItem>
                        <SelectItem value="KG">Kilogramos (KG)</SelectItem>
                        <SelectItem value="LT">Litros (LT)</SelectItem>
                        <SelectItem value="MT">Metros (MT)</SelectItem>
                        <SelectItem value="GL">Galones (GL)</SelectItem>
                        <SelectItem value="CJ">Cajas (CJ)</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
                  ...
                  <FormItem>
                    <FormControl>
                      <ArticleFileUploader
                        ...
                        folder="articulos/imagenes"
                        onAfterRemove={async () => {
                          if (initialData?.id) {
                            // Enviar todos los valores actuales para evitar errores de validación (ej: unit)
        ...
        <div
          className={
            inDialog
              ? "sticky -bottom-4 md:-bottom-6 bg-white dark:bg-slate-950 pt-4 border-t mt-6 pb-4 z-20"
        ...
        >
          <div className={cn("flex w-full items-center gap-2", inDialog ? "flex-col sm:flex-row items-stretch sm:items-center justify-end" : "lg:justify-end")}>
            {onCancel && (
            ...
            <Button type="submit" disabled={isPending} className="w-full bg-[#283c7f] text-white hover:bg-[#24366f] sm:w-auto order-1 sm:order-2">
