...
import { Search, X, Settings, Settings2, Building2, MapPin, HardHat, Briefcase, Factory, Wrench, Users, Truck, Workflow } from "lucide-react";
...
interface ConfigItem {
  ...
  description: string;
  href: string;
  iconName: string; // Cambio a string para serialización
  color: string;
  bg: string;
  border: string;
}
...
export function ConfigurationDashboard({ categories }: ConfigurationDashboardProps) {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-5 pb-2">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-[#1e3a6e]/10 dark:bg-blue-900/30 flex items-center justify-center shrink-0">
              <Settings className="w-6 h-6 text-[#1e3a6e] dark:text-blue-400" />
            </div>
            <div>
              <h1 className="text-xl lg:text-3xl font-bold text-slate-900 dark:text-white leading-tight">
                Configuración <span className="hidden lg:inline">de Mantenimiento</span>
              </h1>
              <p className="text-sm lg:text-lg text-muted-foreground uppercase lg:normal-case font-bold lg:font-normal tracking-wider lg:tracking-normal">Mantenedores y parámetros del sistema</p>
            </div>
          </div>

          <div className="relative w-full lg:max-w-md group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground group-focus-within:text-[#283c7f] dark:group-focus-within:text-blue-400 transition-colors" />
            <Input
              placeholder="Buscar mantenedor..."
              className="pl-10 pr-10 h-11 lg:h-12 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700/50 rounded-xl focus-visible:ring-[#283c7f] dark:focus-visible:ring-blue-500 shadow-sm transition-all"
              value={search}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                {category.items.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group flex flex-col justify-between rounded-xl border bg-white dark:bg-slate-900/50 p-5 shadow-sm transition-all hover:shadow-lg hover:border-[#283c7f]/50 dark:hover:border-blue-500/50 hover:-translate-y-1 overflow-hidden relative border-slate-200 dark:border-slate-800"
                  >
                    <div className="space-y-4 relative z-10">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-[#283c7f] dark:group-hover:text-blue-400 transition-colors">{item.title}</h3>
                        <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed line-clamp-2">{item.description}</p>
                      </div>
                    </div>
                    <div className="absolute inset-0 bg-linear-to-br from-transparent to-transparent group-hover:from-[#283c7f]/5 dark:group-hover:from-blue-500/5 transition-all duration-500 z-0" />
                  </Link>
                ))}
              </div>
            </section>
