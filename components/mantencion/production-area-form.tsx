...
import { mntProductionAreaSchema } from "@/lib/validations/mantencion";
import { Button } from "@/components/ui/button";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage, FormDescription } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Switch } from "@/components/ui/switch";
import { useCreateProductionArea, useUpdateProductionArea, ProductionArea } from "@/lib/hooks/mantencion/use-production-areas";
...
type FormValues = z.infer<typeof mntProductionAreaSchema>;

interface ProductionAreaFormProps {
  mode: "create" | "edit";
  initialData?: ProductionArea | null;
  onCancel: () => void;
  onSuccess: () => void;
}

export function ProductionAreaForm({ mode, initialData, onCancel, onSuccess }: ProductionAreaFormProps) {
  const { mutate: create, isPending: isCreating } = useCreateProductionArea();
  const { mutate: update, isPending: isUpdating } = useUpdateProductionArea();

  const form = useForm<FormValues>({
    resolver: zodResolver(mntProductionAreaSchema),
    defaultValues: {
      name: "",
      description: "",
      isActive: true,
  ...
  const { mnt_production_areas: texts } = maintenanceConfig as any;
        ...
        <CardHeader className="px-0 pb-6">
          <CardTitle className="flex items-center gap-3 text-2xl font-bold">
            <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <Factory className="h-6 w-6 text-[#283c7f] dark:text-blue-400" />
            </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Área Norte" {...field} className="h-11 w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Breve descripción opcional..." {...field} value={field.value || ""} className="h-11 w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-xl border border-dashed p-5 col-span-1 md:col-span-2 bg-slate-50/50 dark:bg-slate-900/20">
                      <div className="space-y-1">
                        ...
                        <FormDescription className="text-sm">Indica si el área está habilitada en el sistema.</FormDescription>
                      </div>
                      <FormControl>
