...
import { mntJobPositionSchema } from "@/lib/validations/mantencion";
import { Button } from "@/components/ui/button";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage, FormDescription } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Switch } from "@/components/ui/switch";
import { useCreateJobPosition, useUpdateJobPosition, JobPosition } from "@/lib/hooks/mantencion/use-job-positions";
...
type FormValues = z.infer<typeof mntJobPositionSchema>;
...
export function JobPositionForm({ mode, initialData, onCancel, onSuccess }: JobPositionFormProps) {
  ...
  const form = useForm<FormValues>({
    resolver: zodResolver(mntJobPositionSchema),
    defaultValues: {
      name: "",
      description: "",
      isActive: true,
  ...
  const onSubmit = (data: FormValues) => {
    if (mode === "create") {
      create(data as Partial<JobPosition>, {
        onSuccess: () => {
          onSuccess();
          form.reset();
        },
      ...
      update(
        { id: initialData?.id!, ...(data as Partial<JobPosition>) },
        {
          onSuccess: () => {
            onSuccess();
            form.reset();
  ...
  const { mnt_job_positions: texts } = maintenanceConfig as any;
        ...
        <CardHeader className="px-0 pb-6">
          <CardTitle className="flex items-center gap-3 text-2xl font-bold">
            <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <Briefcase className="h-6 w-6 text-[#283c7f] dark:text-blue-400" />
            </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Ej: Jefe de Centro, Operador..." {...field} className="h-11 w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Textarea placeholder="Responsabilidades o nivel del cargo..." {...field} value={field.value || ""} className="min-h-[100px] w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-xl border border-dashed p-5 col-span-1 md:col-span-2 bg-slate-50/50 dark:bg-slate-900/20">
                      <div className="space-y-1">
                        ...
                        <FormDescription className="text-sm">Indica si el cargo puede ser asignado a personal.</FormDescription>
                      </div>
                      <FormControl>
