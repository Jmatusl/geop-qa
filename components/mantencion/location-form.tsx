...
import { mntActivityLocationSchema } from "@/lib/validations/mantencion";
import { Button } from "@/components/ui/button";
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage, FormDescription } from "@/components/ui/form";
import { Input } from "@/components/ui/input";
import { Switch } from "@/components/ui/switch";
import { useCreateLocation, useUpdateLocation, ActivityLocation } from "@/lib/hooks/mantencion/use-locations";
import { Navigation, Loader2, Save, X } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import maintenanceConfig from "@/lib/config/ui/maintainers.json";
import { z } from "zod";

type FormValues = z.infer<typeof mntActivityLocationSchema>;

interface LocationFormProps {
  mode: "create" | "edit";
  initialData?: ActivityLocation | null;
  onCancel: () => void;
  onSuccess: () => void;
}

export function LocationForm({ mode, initialData, onCancel, onSuccess }: LocationFormProps) {
  const { mutate: create, isPending: isCreating } = useCreateLocation();
  const { mutate: update, isPending: isUpdating } = useUpdateLocation();

  const form = useForm<FormValues>({
    resolver: zodResolver(mntActivityLocationSchema),
    defaultValues: {
      name: "",
      commune: "",
      isEnabled: true,
  ...
  const onSubmit = (data: FormValues) => {
    if (mode === "create") {
      create(data as Partial<ActivityLocation>, {
        onSuccess: () => {
          onSuccess();
          form.reset();
        },
      ...
      update(
        { id: initialData?.id!, ...(data as Partial<ActivityLocation>) },
        {
          onSuccess: () => {
            onSuccess();
            form.reset();
  ...
  const { mnt_activity_locations: texts } = maintenanceConfig as any;
        ...
        <CardHeader className="px-0 pb-6">
          <CardTitle className="flex items-center gap-3 text-2xl font-bold">
            <div className="p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <Navigation className="h-6 w-6 text-[#283c7f] dark:text-blue-400" />
            </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Ej: Taller Central Puerto Montt" {...field} className="h-11 w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="md:col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="Ej: Puerto Montt..." {...field} value={field.value || ""} className="h-11 w-full" autoComplete="off" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-xl border border-dashed p-5 col-span-1 md:col-span-2 bg-slate-50/50 dark:bg-slate-900/20">
                      <div className="space-y-1">
                        ...
                        <FormDescription className="text-sm">Indica si el lugar está disponible para solicitudes.</FormDescription>
                      </div>
                      <FormControl>
