...
export function useAreas(page: number = 1, limit: number = 10, search: string = "") {
    return useQuery<AreasResponse>({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(`/api/v1/organization/areas?${params.toString()}`);
...
export function useAllAreas() {
    return useQuery<Area[]>({
        queryKey: ["areas", "all"],
        queryFn: async () => {
            const res = await fetch("/api/v1/organization/areas?all=true");
...
export function useCreateArea() {
    ...
    return useMutation({
        mutationFn: async (data: Partial<Area>) => {
            const res = await fetch("/api/v1/organization/areas", {
                method: "POST",
...
export function useUpdateArea() {
    ...
    return useMutation({
        mutationFn: async ({ id, ...data }: { id: string } & Partial<Area>) => {
            const res = await fetch(`/api/v1/organization/areas/${id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(data),
            });
