...
export default async function EditarSolicitudInternaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [request, warehouses] = await Promise.all([
    prisma.bodegaTransaction.findUnique({
      where: { id },
      include: {
        warehouse: true,
        items: {
  ...
  const initialData = {
    ...
    items: await Promise.all(
      request.items.map(async (item) => {
        const itemWarehouseId = request.warehouseId;
    ...
    fotosEvidencia: (request.metadata as any)?.fotosEvidencia || [],
  };
