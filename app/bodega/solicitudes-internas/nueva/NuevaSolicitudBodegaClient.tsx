...
export default function NuevaSolicitudBodegaClient({ warehouses, articles }: Props) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(result?.error || "No se pudo crear la solicitud interna");
        return;
      }

      toast.success(`Solicitud creada: ${result.folio}`);
      router.push(`/bodega/solicitudes-internas/${result.id}`);
    } catch (error) {
      console.error("Error creando solicitud interna:", error);
      toast.error("Ocurrió un error inesperado al crear la solicitud");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="w-full space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Nueva Solicitud Interna</h1>
            <p className="text-sm text-muted-foreground">Crear solicitud de artículos desde bodega.</p>
          </div>
        </div>
      </div>
