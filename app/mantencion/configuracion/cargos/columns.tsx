...
import { JobPosition } from "@/lib/hooks/mantencion/use-job-positions";

export type SortDirection = "asc" | "desc" | null;

interface GetColumnsProps {
  onEdit: (item: JobPosition) => void;
  onDelete: (item: JobPosition) => void;
...
export const getJobPositionsColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<JobPosition>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Cargo" sortKey="name" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => <span className="font-semibold text-[#283c7f]">{row.original.name}</span>,
  },
  {
    accessorKey: "description",
    header: "Descripción",
    cell: ({ row }) => <span className="text-muted-foreground text-sm line-clamp-1">{row.original.description || "-"}</span>,
  },
  {
