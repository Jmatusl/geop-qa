...
import { Equipment } from "@/lib/hooks/mantencion/use-equipments";
...
export const getEquipmentsColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<Equipment>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Nombre de Equipo" sortKey="name" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => (
  ...
  },
  {
    ...
    header: "Nro. Serie",
    cell: ({ row }) => <span className="text-muted-foreground text-sm">{row.original.serialNumber || "-"}</span>,
  },
  {
    accessorKey: "system.name",
    header: "Sistema",
    cell: ({ row }) => <span className="text-muted-foreground text-sm">{row.original.system?.name || "-"}</span>,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant={isActive ? "outline" : "destructive"} className={isActive ? "text-green-600 border-green-600" : ""}>
          {isActive ? "Operativo" : "En Falla/Baja"}
        </Badge>
      );
    },
  },
