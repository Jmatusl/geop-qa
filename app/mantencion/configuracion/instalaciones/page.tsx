...
import { useInstallations, useDeleteInstallation, Installation, useInstallation } from "@/lib/hooks/mantencion/use-installations";
import { getInstallationsColumns, SortDirection } from "./columns";
import { InstallationForm } from "@/components/mantencion/installation-form";
...
export default function InstallationsPage() {
  ...
  // @ts-ignore
  const { mnt_instalaciones: texts } = maintenanceConfig;
  ...
  const { data: queryData, isLoading, isFetching, refetch } = useInstallations(page, pageSize, search) as any;
  const { data: itemToEdit, isLoading: isLoadingItem } = useInstallation(editId);
  const { mutate: deleteItem } = useDeleteInstallation();

  const [externalMode, setExternalMode] = useState<"table" | "edit" | undefined>(editId ? "edit" : "table");
  const [externalItem, setExternalItem] = useState<Installation | null | undefined>(undefined);
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...items].sort((a: Installation, b: Installation) => {
  ...
  return (
    <div className="w-full">
      <BaseMaintainer<Installation>
        title={texts?.header?.title || "Instalaciones"}
        description={texts?.header?.description || "Gestione instalaciones."}
        addNewLabel={texts?.actions?.add_new || "Nueva Instalación"}
      ...
      <DeleteAlertDialog
        ...
        title={`¿Eliminar instalación ${deleteTarget?.name}?`}
