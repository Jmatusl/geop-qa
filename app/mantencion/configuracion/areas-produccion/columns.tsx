...
import { ProductionArea } from "@/lib/hooks/mantencion/use-production-areas";

export type SortDirection = "asc" | "desc" | null;

interface GetColumnsProps {
  onEdit: (item: ProductionArea) => void;
  onDelete: (item: ProductionArea) => void;
...
export const getProductionAreasColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<ProductionArea>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Nombre del Área" sortKey="name" currentSort={currentSort} onSort={onSort} />,
  },
  {
    accessorKey: "description",
    header: "Descripción",
    cell: ({ row }) => <span className="text-muted-foreground text-sm">{row.original.description || "N/A"}</span>,
  },
  {
  ...
  },
  {
    ...
    header: () => <SortableHeader title="Creado" sortKey="createdAt" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => {
