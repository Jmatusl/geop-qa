...
import { useProductionAreas, useDeleteProductionArea, ProductionArea, useProductionArea } from "@/lib/hooks/mantencion/use-production-areas";
import { getProductionAreasColumns, SortDirection } from "./columns";
import { ProductionAreaForm } from "@/components/mantencion/production-area-form";
...
export default function ProductionAreasPage() {
  ...
  // @ts-ignore
  const { mnt_areas_produccion: texts } = maintenanceConfig;
  ...
  const { data: queryData, isLoading, isFetching, refetch } = useProductionAreas(page, pageSize, search) as any;
  const { data: itemToEdit, isLoading: isLoadingItem } = useProductionArea(editId);
  const { mutate: deleteItem } = useDeleteProductionArea();

  const [externalMode, setExternalMode] = useState<"table" | "edit" | undefined>(editId ? "edit" : "table");
  const [externalItem, setExternalItem] = useState<ProductionArea | null | undefined>(undefined);
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...items].sort((a: ProductionArea, b: ProductionArea) => {
  ...
  const handleDelete = (item: ProductionArea) => {
    setDeleteTarget(item);
  };

  const [deleteTarget, setDeleteTarget] = useState<ProductionArea | null>(null);
  ...
  return (
    <div className="w-full">
      <BaseMaintainer<ProductionArea>
        title={texts?.header?.title || "Áreas de Producción"}
        description={texts?.header?.description || "Gestione las área de produccións de producción."}
        addNewLabel={texts?.actions?.add_new || "Nueva Área Producción"}
        searchPlaceholder={texts?.actions?.search_placeholder}
        getColumns={(handlers) =>
          getProductionAreasColumns({
            ...handlers,
      ...
      <DeleteAlertDialog
        ...
        title={`¿Eliminar área de producción ${deleteTarget?.name}?`}
