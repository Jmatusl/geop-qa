...
import { TechnicalResponsible } from "@/lib/hooks/mantencion/use-responsibles";

export type SortDirection = "asc" | "desc" | null;

interface GetColumnsProps {
  onEdit: (item: TechnicalResponsible) => void;
  onDelete: (item: TechnicalResponsible) => void;
...
export const getResponsiblesColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<TechnicalResponsible>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Nombre" sortKey="name" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => <span className="font-semibold text-[#283c7f]">{row.original.name}</span>,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="text-muted-foreground text-sm font-medium">{row.original.area?.name || "-"}</span>,
  },
  {
