...
import { useResponsibles, useDeleteResponsible, TechnicalResponsible, useTechnicalResponsible } from "@/lib/hooks/mantencion/use-responsibles";
import { getResponsiblesColumns, SortDirection } from "./columns";
import { ResponsibleForm } from "@/components/mantencion/responsible-form";
...
export default function ResponsiblesPage() {
  ...
  // @ts-ignore
  const { mnt_responsables: texts } = maintenanceConfig;
  ...
  const { data: queryData, isLoading, isFetching, refetch } = useResponsibles(page, pageSize, search) as any;
  const { data: itemToEdit, isLoading: isLoadingItem } = useTechnicalResponsible(editId);
  const { mutate: deleteItem } = useDeleteResponsible();

  const [externalMode, setExternalMode] = useState<"table" | "edit" | undefined>(editId ? "edit" : "table");
  const [externalItem, setExternalItem] = useState<TechnicalResponsible | null | undefined>(undefined);
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...items].sort((a: TechnicalResponsible, b: TechnicalResponsible) => {
  ...
  const handleDelete = (item: TechnicalResponsible) => {
    setDeleteTarget(item);
  };

  const [deleteTarget, setDeleteTarget] = useState<TechnicalResponsible | null>(null);
  ...
  return (
    <div className="w-full">
      <BaseMaintainer<TechnicalResponsible>
        title={texts?.header?.title || "Responsables"}
        description={texts?.header?.description || "Gestione responsables."}
        addNewLabel={texts?.actions?.add_new || "Nuevo Responsable"}
        searchPlaceholder={texts?.actions?.search_placeholder}
        getColumns={(handlers) =>
          getResponsiblesColumns({
            ...handlers,
      ...
      <DeleteAlertDialog
        ...
        title={`¿Eliminar responsable ${deleteTarget?.name}?`}
