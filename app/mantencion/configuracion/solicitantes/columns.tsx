...
import { Applicant } from "@/lib/hooks/mantencion/use-applicants";

export type SortDirection = "asc" | "desc" | null;

interface GetColumnsProps {
  onEdit: (item: Applicant) => void;
...
export const getApplicantsColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<Applicant>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Nombre" sortKey="name" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => (
      <button onClick={() => onEdit(row.original)} className="font-semibold text-[#283c7f] hover:underline text-left transition-all">
        {row.original.name}
      </button>
    ),
  },
  ...
  },
  {
    ...
    header: "Instalaciones Autorizadas",
    cell: ({ row }) => {
      const installations = row.original.installations || [];
      if (installations.length === 0) {
        return <span className="text-muted-foreground text-sm font-medium italic">Todas (Global)</span>;
      }
      return <span className="text-muted-foreground text-sm font-medium">{installations.map((i) => i.name).join(", ")}</span>;
    },
  },
  {
    accessorKey: "jobPosition.name",
    header: "Cargo",
    cell: ({ row }) => <span className="text-muted-foreground text-sm">{row.original.jobPosition?.name || "-"}</span>,
  },
  {
