...
import { useSystems, useDeleteSystem, System, useSystem } from "@/lib/hooks/mantencion/use-systems";
import { getSystemsColumns, SortDirection } from "./columns";
import { SystemForm } from "@/components/mantencion/system-form";
...
export default function SystemsPage() {
  ...
  // @ts-ignore
  const { mnt_sistemas: texts } = maintenanceConfig;
  ...
  const { data: queryData, isLoading, isFetching, refetch } = useSystems(page, pageSize, search) as any;
  const { data: itemToEdit, isLoading: isLoadingItem } = useSystem(editId);
  const { mutate: deleteItem } = useDeleteSystem();

  const [externalMode, setExternalMode] = useState<"table" | "edit" | undefined>(editId ? "edit" : "table");
  const [externalItem, setExternalItem] = useState<System | null | undefined>(undefined);
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...items].sort((a: System, b: System) => {
  ...
  const handleDelete = (item: System) => {
  ...
  return (
    <div className="w-full">
      <BaseMaintainer<System>
        title={texts?.header?.title || "Sistemas"}
        description={texts?.header?.description || "Gestione sistemas."}
        addNewLabel={texts?.actions?.add_new || "Nuevo Sistema"}
      ...
      <DeleteAlertDialog
        ...
        title={`¿Eliminar sistema ${deleteTarget?.name}?`}
