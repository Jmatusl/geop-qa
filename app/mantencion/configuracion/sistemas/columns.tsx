...
import { System } from "@/lib/hooks/mantencion/use-systems";

export type SortDirection = "asc" | "desc" | null;

interface GetColumnsProps {
  onEdit: (item: System) => void;
  onDelete: (item: System) => void;
...
export const getSystemsColumns = ({ onEdit, onDelete, currentSort, onSort }: GetColumnsProps): ColumnDef<System>[] => [
  {
    accessorKey: "name",
    header: () => <SortableHeader title="Sistema" sortKey="name" currentSort={currentSort} onSort={onSort} />,
    cell: ({ row }) => <span className="font-semibold">{row.original.name}</span>,
  },
  {
    accessorKey: "area.name",
    header: "Área",
    cell: ({ row }) => <span className="text-muted-foreground">{row.original.area?.name || "N/A"}</span>,
  },
  {
