...
import { useLocations, useDeleteLocation, ActivityLocation, useLocation } from "@/lib/hooks/mantencion/use-locations";
import { getLocationsColumns, SortDirection } from "./columns";
import { LocationForm } from "@/components/mantencion/location-form";
...
export default function LocationsPage() {
  ...
  // @ts-ignore
  const { mnt_lugares: texts } = maintenanceConfig;
  ...
  const { data: queryData, isLoading, isFetching, refetch } = useLocations(page, pageSize, search) as any;
  const { data: itemToEdit, isLoading: isLoadingItem } = useLocation(editId);
  ...
  const [externalItem, setExternalItem] = useState<ActivityLocation | null | undefined>(undefined);
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...items].sort((a: ActivityLocation, b: ActivityLocation) => {
  ...
  const handleDelete = (item: ActivityLocation) => {
    setDeleteTarget(item);
  };

  const [deleteTarget, setDeleteTarget] = useState<ActivityLocation | null>(null);
  ...
  return (
    <div className="w-full">
      <BaseMaintainer<ActivityLocation>
        title={texts?.header?.title || "Lugares"}
        description={texts?.header?.description || "Gestione lugares."}
        addNewLabel={texts?.actions?.add_new || "Nuevo Lugar"}
      ...
      <DeleteAlertDialog
        ...
        title={`¿Eliminar lugar ${deleteTarget?.name}?`}
