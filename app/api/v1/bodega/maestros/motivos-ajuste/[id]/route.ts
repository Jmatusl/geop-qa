...
import { bodegaAdjustmentReasonService } from "@/lib/services/bodega/adjustment-reason-service";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const record = await bodegaAdjustmentReasonService.findById(id);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await bodegaAdjustmentReasonService.update(id, parsed.data);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await bodegaAdjustmentReasonService.delete(id);
