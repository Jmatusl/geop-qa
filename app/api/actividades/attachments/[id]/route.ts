...
/**
 * DELETE /api/actividades/attachments/[id]
 * Elimina un adjunto de un requerimiento de actividad.
 */
export async function DELETE(
  ...
  try {
    ...
    const attachment = await prisma.actAttachment.findUnique({
      where: { id },
      include: {
        requirement: {
          select: {
    ...
    const isCreator = attachment.requirement.createdById === session.user.id;
    ...
    if (attachment.requirement.isApproved && !permissions.autoriza) {
      return NextResponse.json(
    ...
    await prisma.actAttachment.delete({
      where: { id },
    });

    // TODO: Agregar audit log cuando se solucione el problema de importación

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("[API] Error eliminando adjunto:", error);
    return NextResponse.json(
