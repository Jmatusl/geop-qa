...
/**
 * POST /api/actividades/activity-attachments
 * Guarda un adjunto de actividad en la base de datos.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { activityId, storagePath, publicUrl, fileName, fileSize, mimeType } = parsed.data;
    ...
    const isCreator = activity.requirement.createdById === session.user.id;
    ...
    if (!isCreator && !hasPermission) {
      return NextResponse.json(
        { error: "No tiene permiso para agregar adjuntos a esta actividad" },
        { status: 403 }
      );
    }

    // No permitir agregar adjuntos si el requerimiento está aprobado (solo usuarios autorizados)
    if (activity.requirement.isApproved && !permissions.autoriza) {
      return NextResponse.json(
    ...
    const attachment = await prisma.actActivityAttachment.create({
      data: {
        activityId,
        storagePath,
        publicUrl,
    ...
    console.error("[API] Error guardando adjunto de actividad:", error);
    return NextResponse.json(
